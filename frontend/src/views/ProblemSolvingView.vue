    ...
    <div v-else-if="pageInfo" class="text-center space-y-6">
      ...
      <div class="bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-lg p-6">
        <div class="flex items-start">
          ...
          <div>
            <h3 class="text-lg font-semibold text-green-800 dark:text-green-200 mb-2">Coming Soon</h3>
            <p class="text-green-700 dark:text-green-300">{{ pageInfo.comingSoon }}</p>
          </div>
        </div>
      </div>
    </div>
...
async function fetchPageInfo() {
  ...
  try {
    const response = await pageService.getPageInfo('problem-solving')
    if (response.success) {
